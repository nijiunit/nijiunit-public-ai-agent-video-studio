...
- 最終MP4: `demo.mp4`
- 正式コンテ: `storyboard_approved.xlsx`（全10ショット承認済み）
...
|---|---|---|---|---|---|
| 企画・脚本・コンテ改訂 | OpenAI Codex（セッションの正確な基盤モデルIDはUIから非公開） | 既存の10ショットを、雲の急降下、川の追走、滝横断、草原低空飛行、着地へ再構成 | 利用者の「滝へ近づく」「虹をすり抜ける」「映画の一場面」「風と滝の音」という要望、v001コンテ | 正式確認用`storyboard_approved.xlsx`、生成処理用`storyboard.json`、音響時間軸、検査条件 | S006〜S010を15秒の連続した見せ場へ改訂。首の逆向きと無音の口元を明示的な禁止事項に追加。全10シートのExcelを承認済みに固定 |
