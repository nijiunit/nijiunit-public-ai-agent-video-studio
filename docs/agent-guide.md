...
- State what the user is about to review and what to check, then display a still review image directly in the conversation when supported; otherwise open that exact image itself. Open HTML review pages and videos as the artifacts themselves. Use a folder handoff only for a file that needs a manual app action, and name the exact file rather than its highlight color, for example: “Double-click `storyboard_v001.xlsx`. When it opens, reply: Opened.” Never place multiple images in a folder and refer to one only as “this,” “the right image,” or “the selected image.” Do not claim an artifact was displayed unless the tool actually opened or attached it. Wait before explaining sheet tabs, correction fields, saving, or the next stage.
