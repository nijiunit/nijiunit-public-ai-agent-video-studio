...
Excel、LibreOffice Calc、Numbersが見つかればExcelを選択し、見つからなければ日本語HTMLを選択した状態でフォルダを開きます。AIエージェント自身がエクスプローラー画面と対象ファイルを確認してから、「『確認_絵コンテ.xlsx』をダブルクリックしてください」のように正確なファイル名だけを案内します。利用者へ「開いた」とだけ返す中間報告は求めません。選択色は環境で変わるため説明に使いません。静止画も短く分かる正確なファイル名を伝えます。
